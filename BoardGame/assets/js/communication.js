...
$(function(){
	...
	socket.on('sentence message', function(msg){
		...
		if(msgObj.type == 'g_init'){
			...
			if( msgObj.masterName == masterUserName && msgObj.curUserName != userName){
				var diceNumbers = msgObj.diceNumber;
				var arrNumbers = diceNumbers.split(",");
				var number1 = arrNumbers[0] * 1;
				var number2 = arrNumbers[1] * 1;
				rollDiceNameNumber(msgObj.curUserName, number1, number2);
			}
		...
		} else if( msgObj.type == 'g_answerNumber'){
			if( msgObj.curUserName == userName)return;
			var _number = msgObj.answerNumber;
			$("#answerTable tr").eq(_number).find(".chkArea").addClass("chk");
			if( msgObj.isCorrect == false){
				gotoStep( msgObj.curUserName, -1);
			...
			// checkOthersAnswer( msgObj.curUserName, -1);
		}
	});
	...
	setInterval( function(){
		...
		__user_obj = {"type":"g_init", 'adminName':adminName, 'gameCode': gameCode, "name": userName, "state": gameState};
